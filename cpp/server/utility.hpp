...
namespace SimpleWeb {
  ...
  public:
    ...
    static bool parse(std::istream &orig_stream, std::string &method, std::string &path, std::string &query_string, std::string &version, CaseInsensitiveMultimap &header, std::string &full_request) noexcept {
			// std::istringstream iss;
			// iss << stream.rdbuf();
			std::string full_request2 ( std::istreambuf_iterator<char>( orig_stream ), (std::istreambuf_iterator<char>()) );
			full_request = full_request2;
