...
public:
  class Response : public std::enable_shared_from_this<Response>,
    ...
    Response(std::shared_ptr<Session> session, long timeout_content) noexcept
        : std::ostream(&streambuf), session(std::move(session)),
  ...
  public:
    ...
    void send(const std::function<void(const error_code &)> &callback =
      ...
      auto self = this->shared_from_this(); // Keep Response instance alive
                                            // through the following async_write
      asio::async_write(
    ...
    /// and empty content
    void
    write(StatusCode status_code = StatusCode::success_ok,
    ...
    void
    write(StatusCode status_code, const std::string &content,
          const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      *this << "HTTP/1.1 " << SimpleWeb::status_code(status_code) << "\r\n";
    ...
    void SuperWrite(const std::string &full) { *this << full; }

    /// Convenience function for writing status line, header fields, and content
    void
    write(StatusCode status_code, std::istream &content,
          const CaseInsensitiveMultimap &header = CaseInsensitiveMultimap()) {
      *this << "HTTP/1.1 " << SimpleWeb::status_code(status_code) << "\r\n";
  ...
  public:
    ...
    void close() noexcept {
      ...
      std::unique_lock<std::mutex> lock(
          socket_close_mutex); // The following operations seems to be needed to
                               // run sequentially
  ...
  public:
    Session(std::size_t max_request_streambuf_size,
      ...
      request = std::shared_ptr<Request>(new Request(
          max_request_streambuf_size, this->connection->remote_endpoint));
    }
...
public:
  ...
  /// Returns assigned port. If io_service is not set, an internal io_service is
  /// created instead. Call before accept_and_run().
  unsigned short bind() {
  ...
  /// If you know the server port in advance, use start() instead.
  /// Accept requests, and if io_service was not set before calling bind(), run
  /// the internal io_service instead. Call after bind().
  void accept_and_run() {
    acceptor->listen();
    accept();
...
protected:
  ...
  ServerBase(unsigned short port) noexcept
      : config(port), connections(new std::unordered_set<Connection *>()),
          ...
          if (!ec) {
            std::size_t num_additional_bytes =
                session->request->streambuf.size() - bytes_transferred;
						...
						int parse_status = RequestMessage::parse(
  ...
  void read_chunked_transfer_encoded(
    ...
    asio::async_read_until(
        *session->connection->socket, session->request->streambuf, "\r\n",
        [this, session, chunks_streambuf](const error_code &ec,
              ...
              asio::async_read(
                  *session->connection->socket, session->request->streambuf,
                  ...
                  [this, session, chunks_streambuf,
                   length](const error_code &ec, size_t /*bytes_transferred*/) {
                    session->connection->cancel_timeout();
                    auto lock =
                        session->connection->handler_runner->continue_lock();
                    if (!lock)
                    ...
                    if (!ec) {
                      ...
                      this->read_chunked_transfer_encoded_chunk(
                          session, chunks_streambuf, length);
                    } else if (this->on_error)
                      this->on_error(session->request, ec);
                  });
            } else
  ...
  void read_chunked_transfer_encoded_chunk(
      const std::shared_ptr<Session> &session,
      const std::shared_ptr<asio::streambuf> &chunks_streambuf,
...
template <class socket_type> class Server : public ServerBase<socket_type> {};
